...
describe("safePipe", () => {
  ...
  it("should maintain proper types through the pipeline", () => {
    ...
    const getUserPermissions = (user: User): string[] =>
      user.id === 1 ? ["read", "write"] : ["read"];
    const formatPermissions = (permissions: string[]): string =>
      `User has permissions: ${permissions.join(", ")}`;

    const getUserPermissionsText = safePipe(
      getUser,
      getUserPermissions,
      formatPermissions,
    );
  ...
  // 체인 합성 테스트
  it("should be able to compose with other SafeChain operations", () => {
    const addOne = (x: number) => x + 1;
    const double = (x: number) => x * 2;
